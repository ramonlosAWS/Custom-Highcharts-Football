...
BUTTONS = [
    ...
    },
    {
        ...
        "ds_name": "Team Draft View Selector",
...
def build_plugin_visual(btn, visual_id, sheet_id):
    ...
    # FieldIds use dataset NAME (not ID), matching QuickSight's pattern
    sort_field_id = f"{ds_name}-SortOrder.0.{ts}"
    label_field_id = f"{ds_name}-ButtonLabel.1.{ts}"
    calc_field_id = f"{ds_name}-{btn['calc_field_name']}.2.{ts}"
...
def main():
    ...
    try:
        ...
        print(f"Error: {e}")
        with open("/tmp/failed-def-v3.json", "w") as f:
            json.dump(definition, f, indent=2, default=str)
        print("Saved to /tmp/failed-def-v3.json")
        raise
